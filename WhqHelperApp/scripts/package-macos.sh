...
PREBUILT_MACOS_ICON="$PROJECT_ROOT/resources/logo.icns"
...
# Prefer a versioned .icns file for CI stability. Fall back to local generation.
if [[ -f "$PREBUILT_MACOS_ICON" ]]; then
  ICON_ARGS=(--icon "$PREBUILT_MACOS_ICON")
else
  if [[ ! -f "$SOURCE_ICON" ]]; then
    echo "Falta el icono macOS pregenerado ($PREBUILT_MACOS_ICON) y también el PNG base: $SOURCE_ICON" >&2
    exit 1
  fi
